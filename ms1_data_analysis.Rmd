...
```
Positive effects of pathogenic fungi on richness were found at low and high elevations (Fig. 1) and on density at low elevations. Insects decreased biomass only at mid-elevations, increased diversity at high elevations, and increased richness at high and low elevations. At high elevations, predator removal increased woody plant diversity, richness, and density. Increases in the number of generalist herbivores reduced woody plant biomass and density at low elevations and richness at low and high elevations.
...
Correlations between LRR of insect effects were not very frequent and were observed in 50% of cases. Correlations were found more frequently in Yawan (five out of eight correlations examined) than in Numba (four out of eight) and no significant correlation was found in Wanang. When we also include correlations between predator exclusion (P) and P+A, this ratio becomes significantly higher for Yawan (9 out of 12 correlations). All observed correlations of top predator effects were positively correlated with insect effects, whereas all effects of herbivore admixture were negatively correlated with insect effects.
...
First we analyze only traits ignoring location. Later we can make an attempt to see wether importance of 
...
### Are there differences in trait values between elevations?
...
trait_data <- data.woody.brut %>%
         mutate(fsite = factor(site, levels=c("wanang","numba","yawan")),
                perc_wc = water.cont/wet_leafg) %>%
         filter(treatment == "c",
                perc_wc > 0,
                unified_names != "piper_aduncum")


sla_plot <- ggplot(trait_data, aes(y=log(sla.d.m2kg1), x = fsite)) +
  geom_jitter(alpha = 0.3,pch=19) +
  theme_classic() + ggtitle("SLA")
wc_plot <- ggplot(trait_data, aes(y=logit(perc_wc), x = fsite)) +
  geom_jitter(alpha = 0.3,pch=19) +
  theme_classic()+ ggtitle("Logit[proportional water content]")

jpeg('figures/sla_wc_elevation.jpg')
ggpubr::ggarrange(sla_plot, wc_plot)
dev.off()
```

```{r}
# test with pgls model

trait_data_tl <- trait_data %>%
  filter(unified_names %in% myTree.rand.poly$tip.label)

trait_elevation_model <- gls(
    log(sla.d.m2kg1) ~ fsite,
    data = trait_data_tl,
    method = "ML",
...
# summary(trait_elevation_model)
# plot(trait_elevation_model) # better fit for log

posthoc_test <- predictmeans::predictmeans(trait_elevation_model, "fsite", adj='tukey')$mean_table$Table

posthoc_test2 <- predictmeans::predictmeans(trait_elevation_model2, "fsite", adj='tukey')$mean_table$Table

sla_ph <- ggplot(as.data.frame(posthoc_test), aes(x = fsite, y = Mean)) +
  geom_errorbar(aes(ymin=Mean-SE, ymax=Mean+SE), width=.05) +
  geom_point()

wc_ph <- ggplot(as.data.frame(posthoc_test2), aes(x = fsite, y = Mean)) +
  geom_errorbar(aes(ymin=Mean-SE, ymax=Mean+SE), width=.05) +
  geom_point()

ggpubr::ggarrange(sla_ph, wc_ph, labels = c("SLA", "WC"))
...
traitPredsCommComp <- function (data = data.woody.brut,
  ...
  tab_model(glsmod1, title = paste(st, treatment))
...
traitPredsCommComp(treatment = "f", st = "wanang")
traitPredsCommComp(treatment = "f", st = "numba")
traitPredsCommComp(treatment = "f", st = "yawan")

traitPredsCommComp(treatment = "i", st = "wanang")
traitPredsCommComp(treatment = "i", st = "numba")
traitPredsCommComp(treatment = "i", st = "yawan")
...
```{r}
source('code/model_definitions_pgls_no_herb.R')

moddata_w <- moddata[moddata$site == "wanang", ]
moddata_n <- moddata[moddata$site == "numba", ]
moddata_y <- moddata[moddata$site == "yawan", ]
...
just_traits_w <- gls(
    lrrtb ~ sla.d.m2kg1 + I(sla.d.m2kg1^2) + water.cont + I(water.cont^2),
    data = moddata_w,
    method = "ML",
    correlation = corMartins(2.718282,
                             phy = myTree.rand.poly,
...
# Compare MSE only models with quadratic and 
wanang_mse <- mean(just_traits_w$residuals^2)
numba_mse <- mean(just_traits_n$residuals^2)
yawan_mse <- mean(just_traits_y$residuals^2)


# MSE for models with traits and all other descriptors
full_model_w <- gls(
    lrrtb ~ lsla.d + I(lsla.d^2) + water.cont + I(water.cont^2) +
      richness + labu + lrrsla + lrrwtrc,
    data = moddata_w,
    method = "ML",
    correlation = corMartins(2.718282,
                             phy = myTree.rand.poly,
...
# MSE for models without the traits but all other descriptors
desc_model_w <- gls(
    lrrtb ~ richness + labu + lrrsla + lrrwtrc,
    data = moddata_w,
    method = "ML",
    correlation = corMartins(2.718282,
                             phy = myTree.rand.poly,
...
# Compare MSE only models with quadratic and 
wanang_full_mse <- mean(full_model_w$residuals^2)
numba_full_mse <- mean(full_model_n$residuals^2)
yawan_full_mse <- mean(full_model_y$residuals^2)

# Compare MSE only models with desc
wanang_desc_mse <- mean(desc_model_w$residuals^2)
numba_desc_mse <- mean(desc_model_n$residuals^2)
yawan_desc_mse <- mean(desc_model_y$residuals^2)

mse_data_i = data.frame(Site = factor(c("Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan"),levels = c("Wanang", "Numba", "Yawan")),
           MSE = c(wanang_mse, numba_mse,yawan_mse,                          
                   wanang_full_mse, numba_full_mse, yawan_full_mse,
                   wanang_desc_mse, numba_desc_mse, yawan_desc_mse),
           Model = c("Traits","Traits","Traits",
                     "Traits+Descriptors","Traits+Descriptors","Traits+Descriptors",
                     "Descriptors","Descriptors","Descriptors"),
           Treatment = "Insecticide")

ggplot(mse_data_i, aes(x = Site, y = MSE, fill = Model))+
  geom_bar(stat = 'identity', position = 'dodge')+
  theme_bw() + ggtitle("Insecticide")
...
# 
# just_sla_w <- gls(
#     lrrtb ~ sla.d.m2kg1 + I(sla.d.m2kg1^2),
#     data = moddata_w,
#     method = "ML",
#     correlation = corMartins(2.718282,
#                              phy = myTree.rand.poly,
...
# 
# # Traits are better
# anova(null_w, just_traits_w)
# 
# # Quadratic + two traits is better
# anova(just_traits_w, just_traits_w_l)
# anova(just_traits_w, just_sla_w)
# anova(just_traits_w, just_wc_w)
# anova(just_traits_w, just_sla_w_l)
# anova(just_traits_w, just_wc_w_l)
# 
# RsquareAdj(just_traits_w)
# 
# sjPlot::tab_model(just_traits_w,
#                   just_traits_w_l)
# 
# 
...
# 
# just_traits.q <- gls(
#     lrrtb ~ sla.d.m2kg1 * site + 
#       I(sla.d.m2kg1^2) * site + 
#       water.cont * site + 
#       I(water.cont^2) * site,
#     data = moddata,
#     method = "ML",
...
# 
# just_traits.l <- gls(
#     lrrtb ~ sla.d.m2kg1 * site + water.cont *  site,
#     data = moddata,
#     method = "ML",
#     correlation = corMartins(2.718282,
#                              phy = myTree.rand.poly,
...
# 2.1 Remove SLA
anova(full.model, just.water.quadratic) # full_model

# 2.3 Remove water content?
anova(full.model, just.sla.quadratic) # No significant difference

# 4. Quadratic vs linear
anova(just.sla.quadratic, just.sla.linear) # Quadratic model is still better

# 5. Interaction reduction
anova(just.sla.quadratic, just.sla.quadratic.noint) # Model with interaction is better
...
mse_data_f = data.frame(Site = factor(c("Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan"),levels = c("Wanang", "Numba", "Yawan")),
           MSE = c(wanang_mse, numba_mse,yawan_mse,                          
                   wanang_full_mse, numba_full_mse, yawan_full_mse,
...
ggplot(mse_data_f, aes(x = Site, y = MSE, fill = Model))+
  geom_bar(stat = 'identity', position = 'dodge')+
...
# model with traits does not have a significantly higher likelihood.
...
mse_data_h = data.frame(Site = factor(c("Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan"),levels = c("Wanang", "Numba", "Yawan")),
           MSE = c(wanang_mse, numba_mse,yawan_mse,                          
                   wanang_full_mse, numba_full_mse, yawan_full_mse,
...
ggplot(mse_data_h, aes(x = Site, y = MSE, fill = Model))+
  geom_bar(stat = 'identity', position = 'dodge')+
  theme_bw() + ggtitle("Herbivore")
...
mse_data_p = data.frame(Site = factor(c("Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan",
                    "Wanang", "Numba", "Yawan"),levels = c("Wanang", "Numba", "Yawan")),
           MSE = c(wanang_mse, numba_mse,yawan_mse,                          
                   wanang_full_mse, numba_full_mse, yawan_full_mse,
...
ggplot(mse_data_p, aes(x = Site, y = MSE, fill = Model))+
  geom_bar(stat = 'identity', position = 'dodge')+
  theme_bw() + ggtitle("Predator")

```

```{r}

mse_data_full <- rbind(mse_data_p, mse_data_f, mse_data_i, mse_data_h)

ggplot(mse_data_full, aes(x = Site, y = 1/MSE, fill = Model))+
  geom_bar(stat = 'identity', position = 'dodge')+
  theme_bw() + facet_wrap(~Treatment)

# ggplot(mse_data_full, aes(x = Site, y = MSE, fill = Treatment))+
#   geom_bar(stat = 'identity', position = 'dodge')+
...
# 2.1 Remove SLA
anova(full.model, just.water.quadratic) # Full model is significanlty better

# 2.2 Remove herbivory
anova(full.model, just.sla.quadratic) # Full model still better
...
pslap <- predictAndVisualize(p.mod, 
                    ...
                    "lsla.d",
                    n = 100,
                    titlestring = "Predators",
                    ylabstring = "Log-response ratio of total biomass.",
                    xlabstring = "Log(SLA)")

pwaterp <- predictAndVisualize(p.mod, 
                    p.dD,
                    "water.cont",
                    n = 100,
                    titlestring = "Predators",
...
islap <- predictAndVisualize(i.mod, 
                    i.dD,
                    "lsla.d",
                    n = 100,
                    titlestring = "Insecticide",
                    ylabstring = "Log-response ratio of total biomass.",
                    xlabstring = "Log(SLA)")

# tiff(filename="figures/traits_pgls.tiff",
#      height=2600,
#      width=3*2600,
#      units='px',
...
## LRR correlations
  ...
  If a plant community has a potential to be strongly negatively affected by insects (negative LRR of insects, strong top-down control), then the P+A results in weaker or positive effects of additional generalist herbivores. Top down control seem to be stronger in the highlands and we see more frequent correlation of the LRR's. (Model where Intra-guild predators are being strongly limited by top predators.)
...
If a plant community has the potential to be strongly negatively impacted by insects (negative LRR from insects, strong top-down control), then P+A would result in weaker or positive impacts from additional generalist herbivores. Top-down control appears to be stronger in the highlands and we see more frequent correlation of LRRs. (Model in which intra-guild predators are strongly constrained by top predators). 

However, what exactly happened after the addition can only be speculated here. With strong bottom-up control, there should not be a correlation like in Wanang. I suspect that the spiders were more active in the exclusion areas, causing their effects to be similar to simple exclusion. A few aspects of insect effects on plants may be important here:
If the community is saturated with herbivores, the addition may have negative effects.
Nature of resource limitation by herbivores (relative or absolute). Change in diet, adaptive trade-off, risk avoidance. All of these can only be speculated about. Generalists are worse at consuming each plant biomass, so they may have positive effects on plants, while natural insect communities have negative effects. It is possible that the addition of herbivores causes a disruption in the functioning of the insect community that affects vegetation.
There was no correlation between the effects of the factors on vegetation at the low elevations. The addition and removal of herbivores were not correlated, as expected from the strong bottom-up control.

The simulations showed that it could not simply be an effect of the relationship between insect abundance and its effect (quadratic and linear functions yield a positive relationship).
 The top-down effects of predators and their positive correlation with insect effects can be well explained by the bottom-up control of insects by plants. This was also assumed in the simulations above. Empirically, we observe no correlation between insect abundance and effect strength (lrr_simualtions.R at the end). There is also no correlation between the abundance of a community and its LRR for any of the treatments (lrr_simulations.R). It is worth noting that the plots where herbivores were used were also covered with nets to prevent increased predation, which is known to correlate with increased leaf damage (Katka). Therefore, this cannot be directly interpreted as a simple increase in herbivore pressure, although it should be, assuming a simple chain model and top-down control by top predators.
We do not know the beliefs of the additional predators. Most likely, they did not lead to a permanent increase in species diversity (graph here). However, the damage from herbivores in general has been... The hypothesis I made above, about increased spider activity in the enclosures, is being further explored here. I want to see if the addition of herbivores has a positive correlation with the regular exclosures. A positive correlation would suggest that the addition of herbivores has little to no effect and that spiders responding to a lack of predators control the herbivore population even with increased abundance.


# Supplement

There is generally no evidence for the over-dispersion in the LRR values at individual elevations. 
...
**Table S1**. Test results for over-dispersion all descriptors at three studied elevations.
```{r, echo=FALSE, results='show', fig.show='all', warning=FALSE, message=FALSE}
sjPlot::tab_df(lrr_test_df)
```

Only woody stem density for the predator exclusion showed some evidence of over-dispersion. Including this in statistical model (using *varIdent(form = ~1|site)* as weights in the GLS function) showed significant difference of LRR in Yawan, that was not observed for LM model.
