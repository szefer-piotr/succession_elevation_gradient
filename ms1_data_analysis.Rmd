...
# 2.1 Remove SLA
anova(full.model, q.herb.q.water) # withouth sla is slightly better (AIC)

# 2.2 Remove herbivory
anova(full.model, q.sla.q.water.noint) # Model without herbivory is not better
...
# Predictions and vizualizations!
traitnm <- "lsla.d"
predictAndVisualize <- function (deltaData, traitnm, n=100) {
  ...
  focal.column = seq(from = min(deltaData[, traitnm], na.rm = T),
                     ...
                     length = n)
  other.vals <- t(apply(deltaData[, other_traits], 2, mean, na.rm = T))
  preddata <- cbind(focal.column,
        do.call("rbind", replicate(n, other.vals, simplify = FALSE)))
  colnames(preddata)[1] <- traitnm
  preddata <- rbind(preddata, preddata, preddata)
  preddata$site <- rep(c("wanang", "numba", "yawan"), each = n)
