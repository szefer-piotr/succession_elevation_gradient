...
for(site.id in substr(unique(COMMVARS$site),1,1)){
  ...
  p2 <- ggplot(species.dat, aes(x = RDA1*1.1, y = RDA2*1.1)) + 
    geom_text(data = species.dat %>%
                filter(names %in% sign.names), aes(label = names))+
    geom_segment(data = arrow.dat, aes(x = 0, y = 0, 
                                       xend = RDA1, yend = RDA2),
                 arrow = arrow(length=unit(0.30,"cm"), 
                               ends="last", 
...
# 
# # Make a matrix
# for(i in 1:dim(cdatc)[1]){
#   
#   # Compare all other sites to the current i
#   for (j in (1:dim(cdatc)[1])[-i]) {
#     val <- beta.pair.abund(cdatc[c(i,j), ])
#   }
#   
# }
# 
...
#     
#     print(rownames(site.treat.comm))
#     
...
# # prws.comp <- list()
# 
# # For data for comparisons between treatments we need pairwise comps 
# # (control vs treat) - this can be obtained from summary of the betareg
# 
# # library(glmmTMB)
# 
...
# 
# # Plot
# # tiff(filename='figures/beta_part_panel.tif',
# #      height=5600,
# #      width=5600,
# #      units='px',
# #      res=800,compression='lzw')
# beta.part.plot <- ggplot(beta.df, aes(x = fsite, y = vals))+
...
# beta.part.plot + theme_bw()
# 
# # Plot for the component comparisons
# pairwise.comp.res$fsite <- factor(pairwise.comp.res$fsite, levels = c("w","n","y"))
# 
# # tiff(filename='figures/beta_part_panel_treats.tif',
# #      height=5600,
# #      width=5600,
# #      units='px',
# #      res=800,compression='lzw')
# 
# # Alternative boxplot
# beta.part.plot <- ggplot(beta.df, aes(x = treat, y = vals))+
#   geom_boxplot()+
#   # stat_summary(fun.y = mean, na.rm = T,
#   #              geom = "point", size = 3, col = "gray30")+
#   # stat_summary(fun.data = "mean_cl_boot", col = "gray30")+
...
#                 label = label), cex = 6,  col = "red", lwd = 2)+
...
# Function that performs RF analysis for a given treatment

# It queries the lgsData that needs to be preapred using separate code (as above)

getLgsData <- function (comp.trt = "i", data) {
  
  # This function queries the data to compute log
  
}


# Returns:
# 1) Classifier RF table
# 2) Confusion matrix
# 3) Regression tree example graph

runRF <- function (lgsData) {
...
rfDataRF <- rfData %>%
  ...
  dplyr::select(pg.cat, perc.herb, water.cont,
                richness, ltb, lsla.d)
...
Below is a confusion matrix for the RF

```{r, results='asis'}
# Confusion Matrix
confusion_mtx = table(test[, 1], y_pred)
print(knitr::kable(confusion_mtx))
```
...
Lets see how good of a job the regression tree did. 

```{r, results='asis'}

print(knitr::kable(table(C50pred, test$pg.cat)))
...
require(scales)
p1 <- ggplot(siteSLAdf, aes(x = sp.rich, y = cwm.sla, color = treatment))+
  geom_point(aes(size = total.bio)) + 
...
### Can SLA predict change in composition?
...
Explore that visually

#### Insecticide 

##### Change in Biomass: log-respose ratio of a species' biomass
...
# Log transform estimated leaf area
deltaData <- deltaData %>%
...
# Log-response ratios of total biomass vs of sla
...
# Model explorations
simple.sla <- gls(
  lrrtb ~ lsla.d*site + I(lsla.d^2)*site + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

just.sla <- gls(
  lrrtb ~ lsla.d*site + I(lsla.d^2)*site,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )


# AIC(just.sla, simple.sla)
# anova(just.sla, simple.sla) # simple SLA is much bette
# tab_model(simple.sla) # But only the lrr SLA is significant.

# Run the full model.
full.sla <- gls(
  lrrtb ~ lsla.d*site + I(lsla.d^2)*site + water.cont*site + I(water.cont^2)*site + perc.herb*site + I(perc.herb^2)*site + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )
...
full.sla.linear <- gls(
  lrrtb ~ lsla.d*site + water.cont*site + perc.herb*site + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# anova(simple.sla, full.sla.linear) # full SLA linear is much better that the one using SLA
# tab_model(full.sla.linear)

full.sla.linear.noint <- gls(
  lrrtb ~ lsla.d + water.cont + perc.herb + richness + labu + lrrsla + site,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# anova(full.sla.linear, full.sla.linear.noint)
# Removing interactions does not improve the model significantly
# tab_model(full.sla.linear.noint)

# Add interaction only to the water content and sla
full.sla.linear.intslawc <- gls(
  lrrtb ~ lsla.d*site + water.cont*site + perc.herb + richness + labu + lrrsla + site,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# anova(full.sla.linear.noint, full.sla.linear.intslawc)
# No effect
# tab_model(full.sla.linear.noint)

full.sla.linear.intsla <- gls(
  lrrtb ~ lsla.d*site + water.cont + perc.herb + richness + labu + lrrsla + site,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# anova(full.sla.linear.noint, full.sla.linear.intsla)
# Any for of iteraction is not improving the model
tab_model(full.sla.linear.noint) # Thus the simplest model seems to be best.

# I further compare to the full.sla.linear.noint model.
# Only water content is modeled as linear
full.sla.custom <-  gls(
  lrrtb ~ lsla.d*site + I(lsla.d^2)*site + water.cont*site + perc.herb*site + I(perc.herb^2)*site + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# anova(full.sla.linear.noint, full.sla.custom)


full.sla.custom.noint <-  gls(
  lrrtb ~ lsla.d + I(lsla.d^2) + water.cont + perc.herb + I(perc.herb^2) + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

full.sla.linear.noint.nosite <- gls(
  lrrtb ~ lsla.d + water.cont + perc.herb + richness + labu + lrrsla,
                data = deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label),
  method = "ML"
  )

# Withouth a site it is even better
# anova(full.sla.linear.noint, full.sla.linear.noint.nosite)


# The linear model remains the best one.
# plot(full.sla.linear.noint.nosite) # This plot looks best
# tab_model(full.sla.linear.noint.nosite)

```

Table. Results of the selected model for the deltaBio.

```{r}
sjPlot::tab_model(full.sla.linear.noint.nosite)
```

Nevertheless it is not a strong correlation and does not explain a lot of variation in the biomass change.

```{r, fig.cap="Observed vs predicted values for the lsls.d and "}
preddata <- data.frame(lsla.d = seq(from = min(deltaData$lsla.d, na.rm = T), 
                                  to = max(deltaData$lsla.d, na.rm = T), 
                                  length = 100),
                       water.cont = mean(deltaData$water.cont, na.rm = T), 
                       perc.herb= mean(deltaData$perc.herb, na.rm = T), 
                       richness= mean(deltaData$richness, na.rm = T), 
                       labu= mean(deltaData$labu, na.rm = T),
                       lrrsla= mean(deltaData$lrrsla, na.rm = T))

preddatalrrsla <- data.frame(lsla.d = mean(deltaData$lsla.d, na.rm = T),
                       water.cont = mean(deltaData$water.cont, na.rm = T), 
                       perc.herb= mean(deltaData$perc.herb, na.rm = T), 
                       richness= mean(deltaData$richness, na.rm = T), 
                       labu= mean(deltaData$labu, na.rm = T),
                       lrrsla=  seq(from = min(deltaData$lrrsla, na.rm = T), 
                                  to = max(deltaData$lrrsla, na.rm = T), 
                                  length = 100))

visData <- deltaData %>% filter(unified_names %in% myTree.rand.poly$tip.label)

predvals <- AICcmodavg::predictSE.gls(full.sla.linear.noint.nosite, newdata = preddata,
                    newdtype = 'response')

preddata$predvals <- predvals$fit
preddata$predse <- predvals$se.fit

predvalslrrsla <- AICcmodavg::predictSE.gls(full.sla.linear.noint.nosite, newdata = preddatalrrsla,
                    newdtype = 'response')

preddatalrrsla$predvals <- predvalslrrsla$fit
preddatalrrsla$predse <- predvalslrrsla$se.fit

# For lsla.d
p1 <- ggplot(preddata, aes(x = lsla.d, y = predvals)) + 
  geom_line()+
  geom_ribbon(aes(ymin = predvals - predse,
                  ymax = predvals + predse), 
              alpha = 0.1) + 
  geom_point(data = visData, aes(x = lsla.d, y = lrrtb)) + 
  ggtitle("Log(SLA) vs Log-response ratio of a species' biomass")

# For lrrsla
p2 <- ggplot(preddatalrrsla, aes(x = lrrsla, y = predvals)) + 
  geom_line()+
  geom_ribbon(aes(ymin = predvals - predse,
                  ymax = predvals + predse), 
              alpha = 0.1) + 
  geom_point(data = visData, aes(x = lrrsla, y = lrrtb)) +
  ggtitle("Delta(SLA) vs Log-response ratio of a species' biomass")


# ggplot(visData, aes(x = lsla.d, y = lrrtb)) + 
#   geom_point(aes(size = tot_bio, alpha = 0.05), pch = 19)+
#   geom_point(aes(x = lsla.d, y = predvals, col = "red")) + 
#   geom_smooth(aes(x = lsla.d, y = predvals), method = "lm")
...
```{r, include=FALSE}
# The Focused model
# Only the predictive power of SLA, water content and percentage herbivory on deltaBio

# Wanang ----
wdata <- deltaData %>% 
  filter(unified_names %in% myTree.rand.poly$tip.label) %>%
  filter(site == "wanang")

# dd.gls.focsed.w <- nlme::gls(lrrtb ~ lsla.d,
#                      data = wdata,
#                      method = "ML",
...
#                                               fixed = T),
#                      na.action = na.omit)
dd.gls.focsed.w <- nlme::gls(lrrtb ~ lsla.d + water.cont + perc.herb + richness + labu + lrrsla,
                     data = wdata,
                     method = "ML",
                     correlation = corMartins(2.718282, 
                                              phy = myTree.rand.poly, 
...
# Predict values
pred.vals <- AICcmodavg::predictSE.gls(dd.gls.focsed.w, wdata, 
                                       se.fit = T)
wdata <- wdata %>%
  mutate(preds = pred.vals$fit,
         se.preds = pred.vals$se.fit,
         se.upr = preds + se.preds,
         se.lwr = preds - se.preds)

# print the MSE
mse_focused_df <- data.frame(mse = with(wdata, mean((lrrtb - preds))^2),
                             treatment = "Insecticide", site = "Wanang")
...
# dd.gls.focsed.n <- nlme::gls(lrrtb ~ lsla.d,
#                      data = ndata,
#                      method = "ML",
#                      correlation = corMartins(2.718282, 
#                                               phy = myTree.rand.poly, 
...
#                                               fixed = T),
#                      na.action = na.omit)
dd.gls.focsed.n <- nlme::gls(lrrtb ~ lsla.d + water.cont + perc.herb + richness + labu + lrrsla,
                     data = ndata,
                     method = "ML",
                     correlation = corMartins(2.718282, 
                                              phy = myTree.rand.poly, 
...
# Predict values
pred.vals <- AICcmodavg::predictSE.gls(dd.gls.focsed.n, ndata, 
                                       se.fit = T)
ndata <- ndata %>%
  mutate(preds = pred.vals$fit,
         se.preds = pred.vals$se.fit,
...
# print the MSE
mse_focused_df <- rbind(mse_focused_df,  
                        data.frame(mse = with(ndata, mean((lrrtb - preds))^2),
                                   treatment = "Insecticide", site = "Numba"))
...
# The model
# dd.gls.focsed.y <- nlme::gls(lrrtb ~ lsla.d,
#                      data = ydata,
#                      method = "ML",
#                      correlation = corMartins(2.718282, 
#                                               phy = myTree.rand.poly, 
...
dd.gls.focsed.y <- nlme::gls(lrrtb ~ lsla.d + water.cont + perc.herb + richness + labu + lrrsla,
                     data = ydata,
                     method = "ML",
                     correlation = corMartins(2.718282,
                                              phy = myTree.rand.poly,
...
# Predict values
pred.vals <- AICcmodavg::predictSE.gls(dd.gls.focsed.y, ydata, 
                                       se.fit = T)
ydata <- ydata %>%
  mutate(preds = pred.vals$fit,
         se.preds = pred.vals$se.fit,
...
# print the MSE
mse_focused_df <- rbind(mse_focused_df,  
                        data.frame(mse = with(ydata, mean((lrrtb - preds))^2),
                                   treatment = "Insecticide", site = "Yawan"))

library(sjPlot)

tab_model(dd.gls.focsed.w, dd.gls.focsed.n, dd.gls.focsed.y)


# fig.cap = "Empirical and predicted values of a simple model of biomass change vs logarithm of SLA for each studied sites."}

# Plot the relationship and add predicted values
wpredPlot <- ggplot(wdata, aes(y = lrrtb, x = lsla.d))+
  geom_point() +
  geom_line(data = wdata, aes(x = lsla.d, y = preds), lwd = 1,
            lty = 2)+
  # geom_ribbon(aes(ymin=se.lwr, ymax=se.upr), alpha = .15) +
  ggtitle("Wanang 200 m a.s.l.")

npredPlot <- ggplot(ndata, aes(y = lrrtb, x = lsla.d))+
  geom_point() +
  geom_line(data = ndata, aes(x = lsla.d, y = preds), 
            lwd = 1, lty = 2)+
  # geom_ribbon(aes(ymin=se.lwr, ymax=se.upr), alpha = .15) + 
  ggtitle("Numba 750 m a.s.l.")

ypredPlot <- ggplot(ydata, aes(y = lrrtb, x = lsla.d))+
  geom_point() +
  geom_line(data = ydata, aes(x = lsla.d, y = preds), lwd = 1)+
  geom_ribbon(aes(ymin=se.lwr, ymax=se.upr), alpha = .15) + 
  ggtitle("Yawan 1900 m a.s.l.")

ggpubr::ggarrange(wpredPlot, npredPlot, ypredPlot,
                  nrow = 1)
# Plot the relationship and add predicted values
ggplot(rbind(ydata, ndata, wdata), aes(y = lrrtb, x = lsla.d, color = site))+
  geom_point() + 
  geom_line(data = rbind(ydata, ndata, wdata), 
            aes(x = lsla.d, y = preds), lwd = 1)+
  geom_ribbon(aes(ymin=se.lwr, ymax=se.upr), alpha = .15)

sjPlot::tab_df(mse_focused_df)

```

The above shows that focused model, where we used SLA as the only predictor of the plant change in biomass has a declining ability to predict change in biomass at the insecticide plot. 
...
source('code/model_definitions.R')

# Are traits improving the null model

# Model selection.
# 1. Full model (quadratic, and site interactions) vs no traits
# anova(no.traits, full.model)
# anova(no.traits, full.model.noint)
# anova(no.traits, all.linear)
# anova(no.traits, all.linear.noint)

# 2. Individual traits 

# SLA
# Check quadratic
# anova(no.traits, just.sla.quadratic.noint)
# Check linear
# anova(no.traits, just.sla.linear.noint)
# Check quadratic interation
# anova(no.traits, just.sla.quadratic)
# Check linear interaction
# anova(no.traits, just.sla.linear)

# WATER
# Check quadratic
# anova(no.traits, just.water.quadratic.noint)
# Check linear
# anova(no.traits, just.water.linear.noint)
# Check quadratic interation
# anova(no.traits, just.water.quadratic)
# Check linear interaction
# anova(no.traits, just.water.linear)

# HERBIVORY
# Check quadratic
# anova(no.traits, just.herb.quadratic.noint)
# Check linear
# anova(no.traits, just.herb.linear.noint)
# Check quadratic interation
# anova(no.traits, just.herb.quadratic)
# Check linear interaction
# anova(no.traits, just.herb.linear)


# 2. Model with all traits linear/quadratic, what combination?

# quadratic SLA, HERB
# anova(no.traits, q.sla.q.herb)
# anova(no.traits, q.sla.q.herb.noint)
# quadratic SLA, linear HERB
# anova(no.traits, q.sla.l.herb)
# anova(no.traits, q.sla.l.herb.noint)
# linear SLA, quadratic HERB
# anova(no.traits, l.sla.q.herb)
# anova(no.traits, l.sla.q.herb.noint)
# linear SLA, linear HERB
# anova(no.traits, l.sla.l.herb)
# anova(no.traits, l.sla.l.herb.noint)


# quadratic HERB, WATER
# anova(no.traits, q.herb.q.water)
# anova(no.traits, q.herb.q.water.noint)
# quadratic HERB, linear WATER
# anova(no.traits, q.herb.l.water)
# anova(no.traits, q.herb.l.water.noint)
# linear HERB, quadratic WATER
# anova(no.traits, l.herb.q.water)
# anova(no.traits, l.herb.q.water.noint)
# linear HERB, linear WATER
# anova(no.traits, l.herb.l.water)
# anova(no.traits, l.herb.l.water.noint)


# quadratic SLA, WATER
# anova(no.traits, q.sla.q.water)
# anova(no.traits, q.sla.q.water.noint)
# quadratic SLA, linear WATER
# anova(no.traits, q.sla.l.water)
# anova(no.traits, q.sla.l.water.noint)
# linear SLA, quadratic WATER
# anova(no.traits, l.sla.q.water)
# anova(no.traits, l.sla.q.water.noint)
# linear SLA, linear WATER
# anova(no.traits, l.sla.l.water)
# anova(no.traits, l.sla.l.water.noint)
...
# Model selection.
# 1. Full model (quadratic, and site interactions) vs no traits
# anova(no.traits, full.model) # full model is much better
# anova(full.model, full.model.noint)

# interaction model is as good as the no interaction. Keep the model without the interaction

# 2. Is the quadratic relationship significant
# anova(full.model.noint, all.linear)            # AIC is lower for the full model noint
# anova(full.model.noint, all.linear.noint)      # AIC is lower for the full model noint

# Traits have nonlinear relationships with the delta biomass.

# 3. REMOVE SLA
# 3.1 anova(full.model.noint, q.herb.q.water.noint) # withouth sla is slightly better (AIC)
# 3.2 anova(full.model.noint, q.herb.q.water)
# 3.3 anova(full.model.noint, q.herb.l.water.noint) # Full model is than this !!!
# 3.4 anova(full.model.noint, q.herb.l.water)
# 3.5 anova(full.model.noint, l.herb.l.water.noint) # Full model still better
# 3.6 anova(full.model.noint, l.herb.l.water) # 

# Including SLA did not mprove the model significantly
# tab_model(q.herb.q.water.noint)

# But if we were to start first by remving perc.herbivory?
# anova(full.model.noint, q.sla.q.water.noint) # Model without herbivory is better
# anova(full.model.noint, q.sla.q.water)       # Full model is better
# anova(q.sla.q.water, q.sla.q.water.noint)    # No interaction is better
# anova(full.model.noint, q.sla.l.water.noint)   # Full model is still better
# anova(full.model.noint, q.sla.l.water)         # Full model wins again
# anova(full.model.noint, l.sla.l.water.noint)   # And again by AIC criterium
# anova(full.model.noint, l.sla.l.water)         # And again

# But if we were to start first by remving water.content?
# anova(full.model.noint, q.sla.q.herb.noint) # Model with herbivory is much better
# anova(full.model.noint, q.sla.q.herb)       # Full model is better
# anova(q.sla.q.herb, q.sla.q.herb.noint)    # No interaction is better
# anova(full.model.noint, q.sla.l.herb.noint)   # Full model is still better
# anova(full.model.noint, q.sla.l.herb)         # Full model wins again
# anova(full.model.noint, l.sla.l.herb.noint)   # And again by AIC criterium
# anova(full.model.noint, l.sla.l.herb)         # And again

# The above suggests that models without hebivory and sla do better job. Water content is the key trait that helps predict the change in biomass in response to the treatment.

# SLA
# Check quadratic
# anova(full.model.noint, just.sla.quadratic.noint)
# Check linear
# anova(full.model.noint, just.sla.linear.noint)
# Check quadratic interation
# anova(full.model.noint, just.sla.quadratic)
# Check linear interaction
# anova(full.model.noint, just.sla.linear)

# WATER
# Check quadratic
# anova(full.model.noint, just.water.quadratic.noint) # Model with just water content

# Is the quadratic form necessary
# anova(just.water.quadratic.noint, just.water.linear.noint) # no difference but quadratic has lower AIC
...
# anova(full.model, full.model.noint)     # No difference, so we can drop the interaction

# interaction model is as good as the no interaction. Keep the model without the interaction

# 2. Is the quadratic relationship significant
# anova(full.model.noint, all.linear)            # Quadratic is a bit better
# anova(full.model.noint, all.linear.noint)      # All linear no int is more efficient

# Traits have nonlinear relationships with the delta biomass.

# 3. REMOVE SLA

# anova(full.model.noint, q.herb.q.water.noint) # withouth sla is slightly better (AIC)
# anova(full.model.noint, q.herb.q.water)
# anova(full.model.noint, q.herb.l.water.noint) # Linear water and quadratic herbivory works even better
# anova(full.model.noint, q.herb.l.water) # interactions make it worse
# anova(full.model.noint, l.herb.l.water.noint) # Full model still better
# anova(q.herb.l.water.noint, l.herb.l.water.noint) # All linear? Slightly better
# anova(full.model.noint, l.herb.l.water) 

# All linear with interaction seems to work best, but the interaction is not improving the model so I will keep it out.

# Including SLA did not mprove the model significantly
# tab_model(l.herb.l.water.noint)
# anova(no.traits, l.herb.l.water.noint)

# But if we were to start first by remving perc.herbivory?
# anova(full.model.noint, q.sla.q.water.noint) # Model without herbivory is not better
# anova(full.model.noint, q.sla.q.water)       # Interaction makes it worse
...
# The above suggests that models without hebivory and sla do better job. 
# Water content is again the key trait that helps predict the change in biomass in response to the treatment.

# WATER
# Check quadratic
# anova(l.herb.l.water.noint, just.water.quadratic.noint) 
# Model with just water content is worse than that without herbivores

# Is the quadratic form necessary
# anova(l.herb.l.water.noint, just.water.linear.noint) # Herbivory is an important trait 
# anova(l.herb.l.water.noint, just.herb.linear.noint) # Water is an important trait, both should stay in the model
