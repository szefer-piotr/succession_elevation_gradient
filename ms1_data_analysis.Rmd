...
# For each elevation calculate inertia for each treatment. This is in order to 
# evaluate whether some sites have the differences more strongly pronounced
# site.id <- "n"
# SUB.DATA <- COMMDATA[grepl(site.id, rownames(COMMDATA)), ]
# SUB.VARS <- COMMVARS[grepl(site.id, COMMVARS$plotid), ]
# 
# treats.to.consider <- c("i","c","h","f","p")
# sub.data.ttc <- SUB.VARS %>%
#   filter(treatment %in% treats.to.consider)
# 
...
# 
# rdamod <- rda(COMMDATA~f+h+p+i+Condition(garden + site), 
#               data = COMMVARS)
# 
# 
# rda.w <- rda(decostand(SUB.DATA, 
#                        method = 'hellinger')~treatment+Condition(garden), 
#              data = SUB.VARS)

# 2. Fitting SLA surfaces to ordination ----

# WARNING: these are data that haven't been fixed yet


## Species traits for evaluation ----
# c1 <- mainDataT$treatment %in% treats.to.consider
# c2 <- mainDataT$unified_names %in% names(rdamod$colsum)
# c3 <- substr(mainDataT$site,1,1) == site.id
# 
# speciesSLA <- mainDataT[c1 & c2 & c3, c("unified_names", "sla.d.m2kg1")] # Ind sp SLA is it possible?
# 
# summary.sp.sla <- speciesSLA %>%
#   group_by(unified_names) %>%
#   summarise(mean.sla = mean(sla.d.m2kg1, na.rm = T))
# 
# ## CWM for sites ----
...
# }
# 
# cwmFun(mainDataT$sla.d.m2kg1, mainDataT$tot_bio)
# 
# siteSLA <- mainDataT %>%
...
#   summarise(cwm.sla = cwmFun(sla.d.m2kg1, tot_bio))
# 
...
for(site.id in substr(unique(COMMVARS$site),1,1)){
  ...
  # print(cgof[!is.na(cgof) & !is.infinite(cgof)])
  
  # Inertia
  # inertia <- inertcomp(rdamod, proportional = T)
  # inttresh <- inertia[inertia[,2] > 0.2, 2]
  # sign.names <- names(inttresh[!is.na(inttresh)])
  
  # Significant vectors
  # sign.sp <- envfit(rdamod, SUB.DATA)
  # sign.names <- names(sign.sp$vectors$pvals[sign.sp$vectors$pvals < 0.05])
  
  colors <- as.numeric(as.factor(SUB.VARS$treatment))
  
  # rda.dat <- invisible(plot(rdamod))
  ...
  # surfDat <- siteSLAdf[rownames(rdamod$CCA$u),]
  # print(ordisurf(rdamod ~ cwm.sla, data = surfDat, bubble = 5))
  
}

## 3.2 Plot the ordination ----
# tiff(filename='figures/ordination_panel.tif',
#      height=5600,
#      width=5200,
#      units='px',
